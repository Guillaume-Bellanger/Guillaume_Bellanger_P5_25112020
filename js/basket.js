...
function suppressItem(event) {
                ...
                let objetId = event.target.getAttribute("data-id").split(',')[0];

                //trouver la position de l'objet dans le tableau
                let arrayId = JSON.parse(localStorage.getItem('basket'));
                console.log(arrayId);
                let isObjet = arrayId.map(function(objet) { return (objet._id == objetId)}); // map = on parcour
                
                let indexObjet = isObjet.indexOf(true);            
               
               console.log(arrayId);
                //sup l'objet dans le tableau
                arrayId.splice(indexObjet, 1);

                //mettre a jour le localstorage
                
                 localStorage.setItem('basket', JSON.stringify(arrayId));
...
function updateBasketTotalPrice() {
              //recuperer 
              let basketTotalPriceArrayID = JSON.parse(localStorage.getItem('basket'));
              //recup un array avec les SS totaux
               let isPrice = basketTotalPriceArrayID.map(function(objet) { return (objet.price * objet.quantity )});
               console.log(isPrice);
...
function sendFormData(data) {   
 fetch("http://localhost:3000/api/furniture/order", {
        ...
        .then(response => {
           ...
           basket = JSON.parse(localStorage.getItem("basket")); // on redéfinis basket en tableau
